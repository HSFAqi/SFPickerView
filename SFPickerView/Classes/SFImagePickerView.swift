...
public class SFImagePickerView: SFBasePickerView {
    ...
    ///   - callback: 回调
    @discardableResult
    public final class func showImagePickerWithTitle(_ title: String?, appearance: SFPickerImageViewAppearance?, dataType: SFPickerDataType, defaultIndexs: [Int]?, config: SFConfig?, callback: @escaping (([Int], [UIImage?]) -> Void)) -> SFBasePickerView{
        let pickerView = SFImagePickerView(frame: CGRect.zero)
        pickerView.showImagePickerWithTitle(title, appearance: appearance, dataType: dataType, defaultIndexs: defaultIndexs, config: config, callback: callback)
        return pickerView
    }
    /// 【Image】单列+多列+联动，对象方法
    /// - Parameters:
    ...
    ///   - config: 配置
    ///   - callback: 回调
    public final func showImagePickerWithTitle(_ title: String?, appearance: SFPickerImageViewAppearance?, dataType: SFPickerDataType, defaultIndexs: [Int]?, config: SFConfig?, callback: @escaping (([Int], [UIImage?]) -> Void)) {
